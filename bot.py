...
async def setup_webhook(application, webhook_url, max_retries=5):
    """Настройка webhook с поддержкой повторных попыток"""
    for i in range(max_retries):
        try:
            ...
            wait = e.retry_after
            logger.warning(f"Retry after {wait} seconds")
